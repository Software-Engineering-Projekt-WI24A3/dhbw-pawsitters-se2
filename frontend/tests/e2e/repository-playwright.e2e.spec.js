...
test.describe('Repository playwright view', () => {
  ...
  test('should expose consistent repository switch navigation targets', async ({ page }) => {
    ...
    const switcher = page.locator('.repository_switch.repository_switch--page');

    await switcher.getByRole('link', { name: token('de', 'repository.git.label') }).click();
    await expect(page).toHaveURL(/\/repository\/git$/);

    await switcher.getByRole('link', { name: token('de', 'repository.board.label') }).click();
