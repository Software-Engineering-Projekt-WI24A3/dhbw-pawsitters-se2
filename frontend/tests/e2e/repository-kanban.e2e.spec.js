...
test.describe('Repository kanban view', () => {
  test('should render board metrics and matching cards when live issue data exists', async ({ page }) => {
    ...
    if (!firstColumn) {
      ...
      await expect(page.locator('.board_card')).toHaveCount(0);
      await expect(page.locator('.board_legend .repo_avatar')).toHaveCount(0);
      return;
    }

    if (snapshot.board.owners.length > 0) {
      await expect(page.locator('.board_legend .repo_avatar').first()).toBeVisible();
    }
