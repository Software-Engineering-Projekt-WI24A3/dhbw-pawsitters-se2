...
async function main() {
  ...
  assert.ok(results.find((entry) => entry.pageKey === 'search'));
  ...
  assert.ok(results.find((entry) => entry.pageKey === 'myOffers'));
  assert.ok(results.find((entry) => entry.pageKey === 'messages'));
